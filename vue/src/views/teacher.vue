...
export default defineComponent({
  ...
  methods: {
    table() {
      request.get("/teacherInfo?name=" + this.name + "&pageNum=" + this.pageNum + "&pageSize=" + this.pageSize).then(res => {
        if (res.code === '0'){
    ...
    del(id) {
      request.delete("/teacherInfo?id=" + id).then(res => {
        if (res.code === '0'){
    ...
    insert(){
      if (this.form.id != null) {
        request.put("/teacherInfo" , this.form).then(res => {
          if (res.code === '0'){
        ...
        request.post("/teacherInfo" , this.form).then(res => {
          if (res.code === '0') {
            this.dialogFormVisible = false;
    ...
    delSelected() {
      ...
      request.put("/teacherInfo/delSelected" , this.multipleSelection).then(res => {
        if (res.code === '0'){
    ...
    exp(){
      location.href = 'http://localhost:8888/chen/teacherInfo/export';
    },
    ...
    agree() {
      request.put("/teacherInfo/agree" , this.form).then(res => {
        if (res.code === '0'){
          this.$message.success("同意当前老师信息成功");
    ...
    disagree(teacher) {
      request.put("/teacherInfo/disagree" , teacher).then(res => {
        if (res.code === '0'){
          this.$message.warning("不同意当前老师信息成功");
          this.table();
        } else {
          this.$message.error(res.msg);
        }
...
<template>
  <div>
    <div style="margin-bottom: 15px">
      ...
      <el-upload action="http://localhost:8888/chen/teacherInfo/upload" style="display: inline-block; margin-left: 10px" :show-file-list="false" :on-success="successUpload">
    ...
    <div>
      <el-dialog title="请填写信息" :visible.sync="dialogFormVisible" width="30%">
        <el-form :model="form">
          ...
          <el-form-item label="职工号" label-width="15%">
      ...
      <el-dialog title="请给当前同意的老师一个职工号" :visible.sync="dialogFormVisible2" width="30%">
        <el-form :model="form">
          <el-form-item label="职工号" label-width="15%">
            <el-input v-model="form.teacherId" autocomplete="off" style="width: 90%"></el-input>
          </el-form-item>
