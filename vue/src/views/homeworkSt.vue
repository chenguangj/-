...
export default defineComponent({
  ...
  methods: {
    ...
    insert(){
      request.put("/cource/uploadSt?img=" + this.form.img + "&courceId=" + this.form.id , this.user).then(res => {
        if (res.code === '0'){
    ...
    down(img){
      if (img == null){
        this.$message.error("老师没有上传作业，还不能下载")
        return
      }
      location.href = 'http://localhost:8888/chen/files/' + img;
    }
      ...
      <el-table :data="tableData" style="width: 100%" ref="table">
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button type="primary" @click="homework(scope.row)">上交作业</el-button>
            <el-button type="primary" @click="down(scope.row.img)">下载作业</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
